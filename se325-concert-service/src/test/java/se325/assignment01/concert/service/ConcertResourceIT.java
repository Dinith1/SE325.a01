...
public class ConcertResourceIT {
    ...
    public void setUp() {
        ...
        Response response = client.target(WEB_SERVICE_URI + "-test/reset").request().get();
     ...
     * <p>
     * The request should return a 200 response, with the requested concert DTO. The
     * concert DTO should contain all performers and dates for that concert.
     */
    @Test
    ...
    /**
     * A more advanced version of the test above. Makes sure the web service still
     * functions correctly when requesting a concert with multiple performers and
     * dates.
     */
    @Test
    ...
    public void testGetAllConcerts() {
        ...
        List<ConcertDTO> concerts = client.target(WEB_SERVICE_URI + "/concerts").request()
    ...
    /**
     * Tests that all concert summaries are returned when requested. Concert
     * summaries contain only the id, title, and image name for each concert.
     */
    @Test
    public void testGetConcertSummaries() {

        List<ConcertSummaryDTO> concerts = client.target(WEB_SERVICE_URI + "/concerts/summaries").request()
    ...
    /**
     * Tests that a 200 response is returned, along with the correct performer info,
     * when requesting a performer with a given id.
     */
    @Test
    ...
    /**
     * Tests that a 404 response is returned when requesting a nonexistent
     * performer.
     */
    @Test
    ...
    public void testGetAllPerformers() {
        ...
        List<PerformerDTO> performers = client.target(WEB_SERVICE_URI + "/performers").request()
    ...
    /**
     * Tests that a 401 error is returned when an incorrect username is supplied on
     * login, and makes sure that no authentication token is generated.
     */
    @Test
    ...
    /**
     * Tests that a 401 error is returned when an incorrect password is supplied on
     * login, and makes sure that no authentication token is generated.
     */
    @Test
    ...
    /**
     * tests that a 200 response is returned when the correct username and password
     * are supplied on login, and that a cookie named "auth" is generated.
     */
    @Test
    ...
    /**
     * Tests that a 401 error is returned when attempting to book while not logged
     * in, and that no booking is actually made.
     */
    @Test
    public void testAttemptUnauthorizedBooking() {
        ...
        BookingRequestDTO bReq = new BookingRequestDTO(1, LocalDateTime.of(2020, 2, 15, 20, 0, 0), seatLabels);

        // Try to book
        Response response = client.target(WEB_SERVICE_URI + "/bookings").request().post(Entity.json(bReq));
    ...
    /**
     * Tests that a 201 response is returned when making a valid authorized booking,
     * and that the requested seats now are correctly reported as being booked.
     */
    @Test
    public void testMakeSuccessfulBooking() {
        ...
        // Attempt booking
        Response response = attemptBooking(client, 1, LocalDateTime.of(2020, 2, 15, 20, 0, 0), "C5", "C6");
    ...
    /**
     * Tests that a 201 response is returned when making a valid authorized booking,
     * and that the link returned allows the user to correctly navigate to the new
     * booking.
     */
    @Test
    public void testGetOwnBookingById() {
        ...
        // Make booking
        Response bookingResponse = attemptBooking(client, 1, LocalDateTime.of(2020, 2, 15, 20, 0, 0), "C5", "C6");
    ...
    /**
     * Tests that a 403 error is returned when trying to access a booking of another
     * user, even if the correct id is known.
     */
    @Test
    public void testAttemptGetOthersBookingById() {
        ...
        // Get the booking
        Response response = client.target(bookingResponse.getLocation()).request().get();
        ...
        response = client.target(bookingResponse.getLocation()).request().get();
    ...
    /**
     * Test that multiple users are each able to access all of their own bookings.
     * No user should be able to see the bookings of any other user.
     */
    @Test
    public void testGetAllBookingsForUser() {
        ...
        response = attemptBooking(client, 2, LocalDateTime.of(2019, 9, 14, 20, 0, 0), "A1", "A2");
        assertEquals(Response.Status.CREATED.getStatusCode(), response.getStatus());
        ...
        try {
            ...
            response = attemptBooking(user2Client, 3, LocalDateTime.of(2020, 1, 23, 20, 0, 0), "C7", "C8");
            assertEquals(Response.Status.CREATED.getStatusCode(), response.getStatus());

            // Get user 1's bookings
            List<BookingDTO> user1Bookings = client.target(WEB_SERVICE_URI + "/bookings").request()
                    .get(new GenericType<List<BookingDTO>>() {
                    });
            ...
            List<BookingDTO> user2Bookings = user2Client.target(WEB_SERVICE_URI + "/bookings").request()
                    .get(new GenericType<List<BookingDTO>>() {
                    });
    ...
    /**
     * Tests that a 401 error is returned when trying to access any booking
     * information while not authenticated.
     */
    @Test
    ...
    /**
     * Tests that a 400 error is returned when trying to book seats for a date on
     * which a given concert is not scheduled.
     */
    @Test
    public void testAttemptBookingWrongDate() {
        ...
        Response response = attemptBooking(client, 1, LocalDateTime.of(2020, 3, 15, 20, 0, 0), "C5", "C6");
        assertEquals(Response.Status.BAD_REQUEST.getStatusCode(), response.getStatus());
    }

    /**
     * Tests that a 400 error is returned when trying to book seats for a
     * nonexistent concert.
     */
    @Test
    public void testAttemptBookingIncorrectConcertId() {
        ...
        // Attempt booking - should fail with bad request
        Response response = attemptBooking(client, 100, LocalDateTime.of(2020, 2, 15, 20, 0, 0), "C5", "C6");
        assertEquals(Response.Status.BAD_REQUEST.getStatusCode(), response.getStatus());
    }

    /**
     * Tests that a 403 error is returned when trying to book a set of seats, all of
     * which have already been booked. Also makes sure that the original booker of
     * those seats retains those seats, and the new user does not.
     */
    @Test
    public void testAttemptDoubleBooking_SameSeats() {
        ...
        try {
            ...
            response = attemptBooking(user2Client, 1, LocalDateTime.of(2020, 2, 15, 20, 0, 0), "C5", "C6");
            assertEquals(Response.Status.FORBIDDEN.getStatusCode(), response.getStatus());
    ...
    /**
     * Tests that a 403 error is returned when trying to book a set of seats, some
     * of which have already been booked. Also makes sure that the original user
     * retains their booking, and that the second booking is not partially completed
     * (i.e. NO seats from the second booking request should be booked, even if some
    ...
    public void testAttemptDoubleBooking_OverlappingSeats() {
        ...
        try {
            ...
            // Try to make the same booking for user 2 - it should fail.
            response = attemptBooking(user2Client, 1, LocalDateTime.of(2020, 2, 15, 20, 0, 0), "C6", "C7");
            assertEquals(Response.Status.FORBIDDEN.getStatusCode(), response.getStatus());
            assertNull(response.getLocation());
    ...
    /**
     * Tests that the booked seats for a particular concert on a particular date can
     * be queried.
     */
    @Test
    ...
    /**
     * Tests that the unbooked seats for a particular concert on a particular date
     * can be queried.
     */
    @Test
    ...
    /**
     * Tests that all seats for a particular concert on a particular date can be
     * queried.
     */
    @Test
    public void testGetAllSeatsForDate() {
        ...
        List<SeatDTO> seats = client.target(WEB_SERVICE_URI + "/seats/2020-02-15T20:00:00?status=Any").request()
                .get(new GenericType<List<SeatDTO>>() {
                });
    ...
    /**
     * Tests that a 401 error is returned when trying to make a subscription while
     * not authenticated.
     */
    @Test
    public void testUnauthorizedSubscription() throws InterruptedException, ExecutionException, TimeoutException {
        ...
        Future<Response> future = client.target(WEB_SERVICE_URI + "/subscribe/concertInfo").request().async()
                .post(Entity.json(subInfo));
    ...
    /**
     * Tests that a 400 error is returned when trying to make a subscription for a
     * nonexistent concert.
     */
    @Test
    public void testBadSubscription_NonexistentConcert()
     ...
     */
    @Test
    public void testBadSubscription_NonexistentDate()
            throws InterruptedException, ExecutionException, TimeoutException {

        testBadSubscription(1, LocalDateTime.of(2030, 2, 15, 20, 0, 0));
    }

    private void testBadSubscription(long concertId, LocalDateTime date)
            throws InterruptedException, ExecutionException, TimeoutException {
        // Log in
        login(client, "testuser", "pa55word");
    ...
    /**
     * Tests that, when authenticated, a subscription can be made to
     * /subscribe/concertInfo, and that subscribers are then notified when the
     * conditions of their subscription are met. The subscribers won't be notified
     * beforehand by mistake.
     */
    @Test
    public void testSubscription() throws ExecutionException, InterruptedException {
        ...
        LocalDateTime date = LocalDateTime.of(2020, 2, 15, 20, 0, 0);
        ConcertInfoSubscriptionDTO subInfo = new ConcertInfoSubscriptionDTO(1, date, 50);
        Future<ConcertInfoNotificationDTO> future = client.target(WEB_SERVICE_URI + "/subscribe/concertInfo").request()
                .async().post(Entity.json(subInfo), ConcertInfoNotificationDTO.class);
        ...
        try {
            ...
            // Now, wait two seconds to see if we received a sub response - we shouldn't
            // have!
            try {
    ...
    /**
     * Tests that, if subscribed to notifications about a particular concert / date,
     * a user won't receive notifications about unrelated concerts / dates.
     */
    @Test
    public void testSubscriptionForDifferentConcert() throws ExecutionException, InterruptedException {
        ...
        try {
            ...
            // Now, wait two seconds to see if user 1 received a sub response - we shouldn't
            // have (different concert / date).
            try {
                future.get(2, TimeUnit.SECONDS);
                fail(); // Shouldn't have worked.
            } catch (TimeoutException e) {
    ...
    private static Response login(Client client, String username, String password) {
        ...
        return client.target(WEB_SERVICE_URI + "/login").request().post(Entity.json(creds));
    }

    /**
     * Helper method - tries to book entire rows.
     */
    private static Response attemptBooking(Client client, long concertId, LocalDateTime date, char minRow,
            char maxRow) {
    ...
    /**
     * Attempts a booking with the given details, and returns the server's response.
